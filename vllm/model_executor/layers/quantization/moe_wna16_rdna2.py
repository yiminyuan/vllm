...
normalizes AWQ and GPTQ into one canonical layout:
  ...
  scales  [E, N, groups]
  qzeros  [E, N/2, groups] uint8 (has_zp)
            element(n,g) = (byte[n//2,g] >> (n%2*4)) & 0xF
...
class MoeWNA16RDNA2Method(MoeWNA16Method):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        w13 = _to_kernel_layout(
            layer.w13_qweight.data, layer.w13_scales.data, w13_qz, has_zp
        )
        w2 = _to_kernel_layout(
            layer.w2_qweight.data, layer.w2_scales.data, w2_qz, has_zp
        )
...
def _rdna2_fused_moe(
    ...
    # SwiGLU is unbounded, and DeepSeek-V4 ships swiglu_limit=10.0 to bound it:
    # the gate is clamped above and the up projection on both sides before the
    # product. Dropping the limit leaves a token whose w13 output exceeds it to
    # grow without bound instead of saturating, which is rare enough to look
    # sporadic and large enough to wreck that token's logits -- measured at 39x
    # the neighbouring row at layer 40, propagating to garbage at the final
    # position. RoutedExperts carries the value (routed_experts.py), the dense
    # MLP already applies it via SiluAndMulWithClamp, and None simply selects
    # the unclamped kernel for models that ship no limit.
    apply_moe_activation(
        activation,
        act_out,
        w1_out,
