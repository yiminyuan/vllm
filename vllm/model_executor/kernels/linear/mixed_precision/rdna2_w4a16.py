    ...
    def can_implement(cls, c: MPLinearLayerConfig) -> tuple[bool, str | None]:
        ...
        if c.weight_type not in cls.SUPPORTED_QUANT_TYPES:
            return (
                False,
                (
                    f"Quant type ({c.weight_type}) not supported by RDNA2 "
                    f"W4A16 kernel; supported: {cls.SUPPORTED_QUANT_TYPES}"
                ),
            )
        ...
        if c.partition_weight_shape[1] % 8 != 0:
            return (
                False,
                (
                    "Output features must be a multiple of 8 for the RDNA2 W4A16 "
                    "kernel (qzeros packing + 64-bit atomic write)"
                ),
            )
