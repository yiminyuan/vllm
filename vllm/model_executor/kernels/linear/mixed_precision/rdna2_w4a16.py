...
and v_pk_fma_f16 but no bf16 packed dot (v_dot2_f32_bf16) and no WMMA, so this
kernel carries only the fp16 scalar path — the same "+1024" bit-trick dequant
that gives the RDNA3 fp16 path its edge over Exllama, which is instruction-for-
instruction available on RDNA2. The HIP kernel lives in
