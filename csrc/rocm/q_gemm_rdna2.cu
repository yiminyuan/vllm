...
// BLOCK_KN_SIZE = 256: each block covers 256 K elements and THREADS_X*4 = 1024
// N columns; gridDim.z = K / BLOCK_KN_SIZE splits K with atomic accumulation.
// THREADS_X = 256 = 8 waves on RDNA2 wave32. Sizing follows the RDNA3 kernel
// (same wave32 geometry).
...
__global__ void gemm_q4_kernel_rdna2(
  ...
  // 256-element/512-byte alignment that would otherwise bank-collide when a
  // thread reads block_a[0..M_COUNT-1][k] (same k, different m).
...
void launch_gemm_q4_for_mcount(const half* a, const uint32_t* b_q_weight,
  ...
  gemm_q4_kernel_rdna2<M_COUNT><<<grid, block, 0, stream>>>(
...
void launch_gemm_q4(const half* a, const uint32_t* b_q_weight,
  ...
  if (size_m == 1) {
    launch_gemm_q4_for_mcount<1>(a, b_q_weight, b_qzeros, b_scales, b_q_perm, c,
                                 size_m, size_n, size_k, groups, zero_offset,
                                 stream);
  } else if (size_m <= 3) {
    launch_gemm_q4_for_mcount<2>(a, b_q_weight, b_qzeros, b_scales, b_q_perm, c,
                                 size_m, size_n, size_k, groups, zero_offset,
                                 stream);
  } else if (size_m <= 7) {
    launch_gemm_q4_for_mcount<4>(a, b_q_weight, b_qzeros, b_scales, b_q_perm, c,
                                 size_m, size_n, size_k, groups, zero_offset,
                                 stream);
  } else {
    launch_gemm_q4_for_mcount<8>(a, b_q_weight, b_qzeros, b_scales, b_q_perm, c,
                                 size_m, size_n, size_k, groups, zero_offset,
                                 stream);
  }
}
