...
@pytest.mark.parametrize("num_tokens", [1, 8, 16, 17, 128])
@pytest.mark.parametrize("hidden_size", [512, 4096])
def test_mhc_proj_matches_linear(num_tokens, hidden_size):
    """The projection has a short-batch path and a vendor path; both must agree.

    ``num_tokens`` straddles the batch size above which the vendor GEMM is used,
    so this covers the handover as well as each side of it.
    """
    from vllm.model_executor.kernels.mhc.triton import _mhc_proj

    set_random_seed(0)
    hc_mult3 = HC_MULT * 2 + HC_MULT * HC_MULT
    k = HC_MULT * hidden_size
    x = torch.randn((num_tokens, k), dtype=torch.float32, device=DEVICE)
    fn = torch.randn((hc_mult3, k), dtype=torch.float32, device=DEVICE) * 1e-4

    ref = torch.nn.functional.linear(x, fn)
    got = _mhc_proj(x, fn)
...
@pytest.mark.parametrize("hc_mult", [3, 4])
@pytest.mark.parametrize("sinkhorn_repeat", [1, 3, 20])
def test_sinkhorn_triton_matches_torch(hc_mult, sinkhorn_repeat):
    """Fusing the Sinkhorn iterations must not move the result.

    A ``hc_mult`` that is not a power of two exercises the masked lanes, which
    must stay out of the row and column sums.
    """
    import vllm.model_executor.kernels.mhc  # noqa: F401
    ...
    hc_mult3 = hc_mult * 2 + hc_mult * hc_mult
    residual = torch.randn(
        (num_tokens, hc_mult, hidden_size), dtype=torch.float16, device=DEVICE
    )
    fn = (
        torch.randn(
            (hc_mult3, hc_mult * hidden_size), dtype=torch.float32, device=DEVICE
    ...
    layer_input = torch.empty(
        (num_tokens, hidden_size), dtype=residual.dtype, device=DEVICE
    )
    torch.ops.vllm.mhc_pre_triton(
        residual,
        fn,
    ...
    torch.testing.assert_close(comb_mix, ref[1], rtol=1e-5, atol=1e-6)
