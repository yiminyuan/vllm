...
@pytest.mark.skipif(
    not current_platform.is_rocm(), reason="indexer KV path is ROCm-only"
)
@pytest.mark.parametrize("scale_fmt", ["ue8m0", None])
def test_indexer_k_quant_is_identical_either_way(scale_fmt):
    """The indexer KV encoder swap must not change a single cache byte.

    Covers the second production encode site (the sparse-attention indexer),
    alongside the KV-insert test above.
    """
    from unittest.mock import patch
    ...
    torch.manual_seed(0)
    num_tokens, head_dim, block_size, n_blocks = 33, 128, 64, 4
    k = torch.randn(num_tokens, head_dim, dtype=torch.float16, device=device) * 2.0
    slot_mapping = torch.arange(num_tokens, dtype=torch.int64, device=device)

    caches = []
    for bitops in (False, True):
        kv_cache = torch.zeros(
            n_blocks, block_size, head_dim + 4, dtype=torch.uint8, device=device
        )
        with patch.object(mla, "use_fp8_bitops_decode", lambda bitops=bitops: bitops):
            mla.indexer_k_quant_and_cache_triton(
                k, kv_cache, slot_mapping, head_dim, scale_fmt
            )
        caches.append(kv_cache)

    assert torch.equal(caches[0], caches[1])
    assert caches[1].any(), "indexer cache was never written"


def test_prefill_mqa_logits_fallback_is_finite_and_masked():
    """The non-AITER prefill logits path must stay in a natively supported dtype.

    RDNA2 has no bf16 dot product, so the fallback computes in fp32; fp8 inputs
    are exact there and the d-sum cannot overflow the way fp16 would.
    """
    from vllm.v1.attention.ops.rocm_aiter_mla_sparse import fp8_mqa_logits_torch

    torch.manual_seed(0)
    m, n, h, d = 8, 32, 4, 128
    q = (torch.randn(m, h, d, device=device) * 3).to(torch.float8_e4m3fn)
    k = (torch.randn(n, d, device=device) * 3).to(torch.float8_e4m3fn)
    k_scale = torch.rand(n, 1, device=device, dtype=torch.float32) + 0.5
    weights = torch.rand(m, h, device=device, dtype=torch.float32)
    ks = torch.zeros(m, dtype=torch.int32, device=device)
    ke = torch.full((m,), n, dtype=torch.int32, device=device)

    logits = fp8_mqa_logits_torch(q, (k, k_scale), weights, ks, ke)
    assert logits.shape == (m, n) and logits.dtype == torch.float32
    assert torch.isfinite(logits).all()

    # Rows outside [ks, ke) must be masked to -inf.
    ke_half = torch.full((m,), n // 2, dtype=torch.int32, device=device)
    masked = fp8_mqa_logits_torch(q, (k, k_scale), weights, ks, ke_half)
    assert torch.isinf(masked[:, n // 2 :]).all() and (masked[:, n // 2 :] < 0).all()
    assert torch.isfinite(masked[:, : n // 2]).all()
