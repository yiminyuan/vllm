...
# ---------------------------------------------------------------------------
# Split-K combine: fp32 accumulation
# ---------------------------------------------------------------------------
#
# gridDim.z splits K, so every output is a sum of partials combined through
# memory. RDNA2 has no float-add atomic (the ISA offers only FCMPSWAP, FMIN and
# FMAX for floats), so that combine is a CAS loop whatever it accumulates in.
# It used to accumulate in fp16, which rounded each partial to 11 bits and left
# the result dependent on the order the splits happened to land in. Both tests
# below fail against that version.


@gfx1030_only
def test_rdna2_w4a16_is_deterministic():
    """Identical inputs must give identical outputs, bit for bit.

    Nothing varies between these calls -- same tensors, same shapes, one call at
    a time -- so any difference comes from the order the K-splits accumulated
    in. Greedy decoding turns that into flipped argmax ties, i.e. a different
    answer to the same prompt.
    """
    set_random_seed(0)
    M, K, N, group = 1, 4096, 4096, 128
    a = torch.randn(M, K, dtype=torch.float16, device=device) * 0.1
    qw = torch.randint(0, 2**31, (K // 8, N), dtype=torch.int32, device=device)
    qz = torch.zeros((K // group, N // 8), dtype=torch.int32, device=device)
    sc = torch.rand(K // group, N, dtype=torch.float16, device=device) * 0.02 + 0.01
    gidx = torch.empty(0, dtype=torch.int32, device=device)

    first = torch.ops._rocm_C.gptq_gemm_rdna2(a, qw, qz, sc, gidx, True).clone()
    for _ in range(7):
        again = torch.ops._rocm_C.gptq_gemm_rdna2(a, qw, qz, sc, gidx, True)
        assert torch.equal(first, again), "same inputs produced a different result"


@gfx1030_only
@pytest.mark.parametrize("K", [4096, 7168])
def test_rdna2_w4a16_split_k_combine_keeps_precision(K):
    """The split-K combine must not be worse than summing in the output dtype.

    Every 4-bit nibble is 1, every zero-point 0 and every scale 1, so the
    dequantized weight is exactly 1.0 and the GEMM reduces to a row sum of the
    activations -- an exact reference, with no second implementation to be
    wrong. An fp16 row sum of the same terms is the bar: a kernel that hands
    back fp16 cannot beat it, but combining the splits in fp16 landed an order
    of magnitude above it (12x at K=4096, 38x at K=7168), which is real accuracy
    loss rather than the output dtype's own rounding.
    """
    set_random_seed(0)
    M, N, group = 1, 2048, 128
    a = torch.randn(M, K, dtype=torch.float16, device=device) * 0.1
    qw = torch.full((K // 8, N), 0x11111111, dtype=torch.int32, device=device)
    qz = torch.zeros((K // group, N // 8), dtype=torch.int32, device=device)
    sc = torch.ones(K // group, N, dtype=torch.float16, device=device)
    gidx = torch.empty(0, dtype=torch.int32, device=device)

    out = torch.ops._rocm_C.gptq_gemm_rdna2(a, qw, qz, sc, gidx, True)
    exact = a.to(torch.float32).sum(dim=1, keepdim=True).expand(M, N)
    fp16_sum = a.sum(dim=1, keepdim=True).to(torch.float32).expand(M, N)

    kernel_err = (out.to(torch.float32) - exact).abs().max().item()
    fp16_err = (fp16_sum - exact).abs().max().item()
    # A little slack over the fp16 row sum: the kernel rounds the scales and the
    # dequantized weight too, which the reference sum does not.
    assert kernel_err <= 4 * fp16_err + 1e-3, (
        f"K={K}: kernel error {kernel_err:.5f} against {fp16_err:.5f} for an "
        "fp16 row sum of the same terms -- the split-K combine is losing "
        "precision beyond the output dtype"
    )


@gfx1030_only
def test_rdna2_w4a16_multi_pass_m_tiling_matches_a_row_split():
    """Tall problems split M into passes; the seams must not move the answer.

    One scratch slice per K-split costs num_splits * M * N floats, so instead of
    dropping to the order-dependent CAS on large M the launcher processes rows
    in passes that fit its budget, offsetting the input and output by whole
    rows. Splitting the same problem by rows by hand must give the identical
    result, bit for bit.
    """
    torch.manual_seed(0)
    device = "cuda"
    M, K, N, group = 1024, 4096, 4096, 128
    qw = torch.randint(0, 2**31, (K // 8, N), dtype=torch.int32, device=device)
    sc = torch.rand(K // group, N, dtype=torch.float16, device=device) * 0.02 + 0.01
    qz = torch.zeros((K // group, N // 8), dtype=torch.int32, device=device)
    gidx = torch.empty(0, dtype=torch.int32, device=device)
    inp = torch.randn(M, K, dtype=torch.float16, device=device) * 0.1

    full = torch.ops._rocm_C.gptq_gemm_rdna2(inp, qw, qz, sc, gidx, True)
    halves = [
        torch.ops._rocm_C.gptq_gemm_rdna2(
            inp[s].contiguous(), qw, qz, sc, gidx, True
        )
        for s in (slice(0, M // 2), slice(M // 2, M))
    ]
    assert torch.equal(full, torch.cat(halves, dim=0)), (
        "multi-pass M tiling disagrees with an equivalent row split"
    )
